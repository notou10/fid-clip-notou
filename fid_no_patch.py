...
# def compute(real_dir, generated_dir, img_num):
...
#                 fake_dataset, batch_size=bs, shuffle=False, num_workers=4)
...
#             if sum([a.shape[0] for a in real_clip_features]) == img_num :break
...
#     print(fid_imgnet, fid_clip)
    ...
    # f = open(f"./output/no_patch/{real_dir.split('/')[-2]}.txt", 'a')
...
# pathh = "/mnt/hdd/dongkyun/PROJECTS/fid_clip/salient/lsun_fused/set1"
# task_list = os.listdir(pathh)
# for task in task_list:
#     dataset1 = os.path.join(pathh, task)
    
#     real_dir = "/mnt/hdd/dongkyun/PROJECTS/fid_clip/salient/lsun_cat/gt_2"
#     generated_dir = dataset1    
#     out_folder = f"./np_folder/{real_dir.split('/')[-1]}*{generated_dir.split('/')[-1]}"
...
#                                 batch_size=bs,
#                                 num_images=real_dir)
compute(real_dir, generated_dir, 10000, 200)
    
# out_folder = f"./np_folder/{real_dir.split('/')[-1]}*{generated_dir.split('/')[-1]}"    
# if not os.path.isdir(out_folder):
#         os.mkdir(out_folder)
        
# compute(real_dir, generated_dir)
